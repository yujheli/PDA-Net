...
class RandomPosSampler(Sampler):
    def __init__(self, data_source, neg_pos_ratio=1):
        super(RandomPosSampler, self).__init__(data_source)
        self.data_source = data_source
        self.num_samples = len(data_source)
        self.neg_pos_ratio = neg_pos_ratio
        # Sort by pid
    ...
    def __iter__(self):
        ...
        for i in indices:
            ...
            # positive sample
            start, end = self.index_range[pid]
            pos_indices = _choose_from(start, end, excluded_range=(i, i), size=self.neg_pos_ratio, replace=True)
