...
class DownsampleScale(object):
    def __init__(self, height, width, d=2, interpolation=Image.BILINEAR):
        self.height = height
        self.width = width
        self.interpolation = interpolation
        self.d = d
...
#         img = img[::2,::2]
        ...
        if h == self.height and w == self.width:
            return img
        img_ =  img.resize((int(self.width/self.d), int(self.height/self.d)), self.interpolation)
        return img_.resize((self.width, self.height), self.interpolation)
